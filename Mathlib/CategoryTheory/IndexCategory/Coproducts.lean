...
def sum_to_initial {m n : IndexCategory} (i : Fin (m + n).len) (h : i.val < m.len) : Fin m.len :=
  ⟨i.val, h⟩

def sum_to_concluding {m n : IndexCategory} (i : Fin (m + n).len)
    (h : ¬ i.val < m.len) : Fin n.len :=
  ⟨i.val - m.len, (Nat.sub_lt_iff_lt_add' (Nat.le_of_not_lt h)).mpr (i.isLt.trans_eq add_len)⟩

@[simp]
theorem sum_to_initial_val {m n : IndexCategory} (i : Fin (m + n).len) (h : i.val < m.len) :
    (sum_to_initial i h).val = i.val :=
  rfl

@[simp]
theorem sum_to_concluding_val {m n : IndexCategory} (i : Fin (m + n).len) (h : ¬ i.val < m.len) :
    (sum_to_concluding i h).val = i.val - m.len :=
  rfl
...
@[simp]
theorem ι₂_toFun_sum_to_concluding_id {m n : IndexCategory} (i : Fin (m + n).len)
    (h : ¬ i.val < m.len) : ι₂.toFun (sum_to_concluding i h) = i :=
  by ext ; simp [Nat.le_of_not_lt h]

@[simp]
theorem sum_to_initial_ι₁_toFun_id {m n : IndexCategory} (i : Fin m.len)
    (h : ((@ι₁ _ n).toFun i).val < m.len) : sum_to_initial (ι₁.toFun i) h = i :=
  by ext ; simp

@[simp]
theorem sum_to_concluding_ι₂_toFun_id {m n : IndexCategory} (i : Fin n.len)
    (h : ¬ ((@ι₂ m _).toFun i).val < m.len) : sum_to_concluding (ι₂.toFun i) h = i :=
  by ext ; simp
...
def match_hom {m n k : IndexCategory} (f : m ⟶ k) (g : n ⟶ k) : m + n ⟶ k :=
  Hom.mk <| fun i ↦ if h : i.val < m.len then f.toFun (sum_to_initial i h)
    else g.toFun (sum_to_concluding i h)

scoped notation:1050 "[" f ", " g "]" => match_hom f g

@[simp]
theorem match_hom_apply_val {m n k : IndexCategory} (f : m ⟶ k) (g : n ⟶ k) (i : Fin (m + n).len) :
    ([f, g].toFun i).val = if h : i.val < m.len then (f.toFun (sum_to_initial i h)).val
    else (g.toFun (sum_to_concluding i h)).val :=
  by unfold match_hom ; if h : i.val < m.len then simp [h] else simp [h]

@[simp]
theorem match_hom_apply_left {m n k : IndexCategory} {f : m ⟶ k} {g : n ⟶ k}
    {i : Fin (m + n).len} (h : i.val < m.len) : [f, g].toFun i = f.toFun (sum_to_initial i h) :=
  by unfold match_hom ; simp [h]

@[simp]
theorem match_hom_apply_right {m n k : IndexCategory} {f : m ⟶ k} {g : n ⟶ k} {i : Fin (m + n).len}
    (h : ¬ i.val < m.len) : [f, g].toFun i = g.toFun (sum_to_concluding i h) :=
  by unfold match_hom ; simp [h]
...
theorem id_sum_id (m n : IndexCategory) : 𝟙 m ++ 𝟙 n = 𝟙 (m + n) :=
  Eq.symm <| match_uniq (by simp) (by simp)
...
theorem sum_ids {n : IndexCategory} (ms : Fin n.len → IndexCategory) :
  ...
  Eq.symm <| match_homs_uniq _ _ fun _ ↦ by simp
